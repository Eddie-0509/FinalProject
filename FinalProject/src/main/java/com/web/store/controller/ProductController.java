...
public class ProductController {
	...
	public String processAddNewProductForm(@ModelAttribute("bookBean") BookBean bb, BindingResult result) {
		CompanyBean cb = productService.getCompanyById(bb.getCompanyId());
		String[] suppressedFields = result.getSuppressedFields();
		if (suppressedFields.length > 0) {
			throw new RuntimeException("嘗試傳入不允許欄位" + StringUtils.arrayToCommaDelimitedString(suppressedFields));
		}
	...
	@ModelAttribute("companyList") // modelAttribute 會先執行
